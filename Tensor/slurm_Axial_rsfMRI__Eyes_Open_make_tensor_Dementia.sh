#!/bin/bash
#SBATCH --job-name=slurm_Axial_rsfMRI__Eyes_Open_make_tensor_Dementia
#SBATCH --partition=cpu
#SBATCH --nodelist=node01
#SBATCH --cpus-per-task=4
#SBATCH --mem=0
#SBATCH --time=3-23:59:59
#SBATCH --output=slurm_Axial_rsfMRI__Eyes_Open_make_tensor_Dementia.log
#SBATCH --error=slurm_Axial_rsfMRI__Eyes_Open_make_tensor_Dementia.log

# sudo sbatch slurm_Axial_rsfMRI__Eyes_Open_make_tensor_Dementia.sh
...
# Run the container
echo "Running the making fALFF Tensor (fMRI)..."
docker run --rm \
    ...
    bash -i -c "source ~/.bashrc && \
                ...
                Rscript /root/Project/ADNI/Tensor/make_tensor_Dementia.R"
...
if [[ $? -ne 0 ]]; then
    ...
    echo "making fALFF Tensor (fMRI) completed successfully!"
fi
